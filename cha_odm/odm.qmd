...
<a
href="/slides/cha_odm/odm.html"
target="_blank"><img
src="https://img.shields.io/badge/slides-decision-yellow"></a>
...
- We can make optimal decisions with cost-sensitive classification
- In order to make a classification decision we need to know the cost of making
  a wrong prediction.
- If in addition we have access to the probability of the different outcomes,
  we can make optimal decisions.
- This is very important in critical applications: e.g. medical diagnosis.
- The prior distributions may be adjusted during deployment.
- Assume the costs are not instance dependent (only depend on the true class).
...
predicted class $C_i$ and its cost of missclassification and correct
classification. For example, the following cost matrix
...
class that minimizes the expected cost.
...
with the equality having the same expected missclassification independent on
the predicted class.
...
In the binary classification setting we can derive the optimal threshold of
selecting class one if $p \ge t^*$.

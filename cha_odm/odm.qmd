...
- Even if we know a class prior change during deployment

## Binary example

- The following is a typical example of a cost matrix for a binary problem
...
- Given the posterior probabilities $P(C_i|\vec{x})$ and the cost matrix we can
  calculate the expected cost of predicting class $i$
...
ax = fig.add_subplot()
ax.set_title(f"Expected cost of predicting \nthe optimal class")
for i in range(len(C)):
    def cost_func(prob):
        expected_costs = np.inner(prob, C)
        min_p_id = np.argmin(expected_costs)

...
## Why Optimal Decision Making {.smaller}
...
![Example of critical applications](images/case_examples.svg)
...
- **Class estimation with option of abstaining**: Outputs a class prediction or abstains [@coenen2020].
- **Rankings estimation**: Outputs a ranked list of possible
  classes [@brinker2020].
- **Score surrogates**: Outputs a continuous score which is commonly surrogates
  for the task of classification (e.g. Support Vector Machines).
- **Probability estimation**: Outputs class posterior probability
  ...
  proportions (e.g. decision trees, random forests, k-nearest neightbour).
- **Others**: For example methods that use possibility theory [@dubois2001],
  credal sets [@levi1980] or conformal predictions [@vovk2005].

![Classifiers as a black box](images/classifiers.svg)

# Cost-sensitive classification

- Cost-sensitive classification [@elkan2001] provides a framework to make
  optimal decisions (with some restrictions).
...
- If in addition we have access to the probability of the different outcomes,
  we can make optimal decisions.
...
Given the posterior probabilities $P(C_i|\vec{x})$ and the cost matrix $c$ we
can calculate the expected cost of predicting class $C_i$
...
plt.legend()
plt.annotate("Cost = 0.5", (0.5, 0.48), xytext=(0.5, 0.2),
...
2. **Class comination**: One class does not consistently have lower costs than other classes
...
## Optimal Decision {.smaller}

::: {.fragment fragment-index=1}
If we know the posterior probabilities, the optimal decision is to choose the
class that minimizes the expected cost which can be obtained by marginalising
the predicted class over all possible true classes.
...
- Predicting **Class 1** will have an expected gain of $5 \times 0.4 - 1 \times
0.6 = \mathbf{\color{darkgreen}{1.4}}$
- Predicting **Class 2** will have an expected gain of $-10 \times 0.4 + 1 \times
...
classes is $1$ and a cost of $0$ for its correct prediction.
...
if shiffted by $+7$ and scaled by $1/20$ results in 
...
However, with more than 2 classes the threshold is not a single value but a
decision boundary in the probability simplex.
...
### Ternary hyperplanes per decision
...
### Ternary hyperplanes optimal decision
...
### Option to abstain optimal costs
